...
class Coleccion:
    ...
    def editar_album(self, album_id, titulo, anio, descripcion, medio):
        busqueda = (
            session.query(Album)
            .filter(Album.titulo == titulo, Album.id != album_id)
            .all()
        )
        if len(busqueda) == 0:
    ...
    def dar_interpretes_de_album(self, album_id):
        canciones = (
            ...
            .filter(Cancion.albumes.any(Album.id.in_([album_id])))
            .all()
        )
        interpretes = []
    ...
    def buscar_albumes_por_titulo(self, album_titulo):
        albumes = [
            elem.__dict__
            for elem in session.query(Album)
            .filter(Album.titulo.ilike("%{0}%".format(album_titulo)))
            .all()
        ]
        return albumes
    ...
    def editar_cancion(
        self, cancion_id, titulo, minutos, segundos, compositor, interpretes
    ):
        busqueda = (
            session.query(Cancion)
            .filter(Cancion.titulo == titulo, Cancion.id != cancion_id)
            .all()
        )
        if len(busqueda) == 0:
    ...
    def dar_cancion_por_id(self, cancion_id):
        ...
        cancion_dict["interpretes"] = [
            self.dar_interprete_por_id(interprete.id)
            for interprete in cancion.interpretes
        ]
        return cancion_dict
    ...
    def buscar_canciones_por_titulo(self, cancion_titulo):
        canciones = [
            elem.__dict__
            for elem in session.query(Cancion)
            .filter(Cancion.titulo.ilike("%{0}%".format(cancion_titulo)))
            .all()
        ]
        return canciones
    ...
    def buscar_canciones_por_interprete(self, interprete_nombre):
        if interprete_nombre == "":
            ...
            canciones = (
                session.query(Cancion)
                .filter(
                    Cancion.interpretes.any(
                        Interprete.nombre.ilike("%{0}%".format(interprete_nombre))
                    )
                )
                .all()
            )
    ...
    def editar_interprete(self, interprete_id, nombre, texto_curiosidades):
        busqueda = (
            session.query(Interprete)
            .filter(Interprete.id != interprete_id, Interprete.nombre == nombre)
            .all()
        )
        if len(busqueda) == 0:
            interprete = (
                session.query(Interprete).filter(Interprete.id == interprete_id).first()
            )
    ...
    def agregar_album_duplicado(self, titulo, anio, descripcion, medio):
        """
        Función duplicada para agregar un álbum, con lógica idéntica a agregar_album.
        """
        busqueda = session.query(Album).filter(Album.titulo == titulo).all()
        if len(busqueda) == 0:
            album = Album(titulo=titulo, ano=anio, descripcion=descripcion, medio=medio)
    ...
    def eliminar_album_duplicado(self, album_id):
        """
        Función duplicada para eliminar un álbum, con lógica idéntica a eliminar_album.
        """
        try:
            album = session.query(Album).filter(Album.id == album_id).first()
            session.delete(album)
    ...
    def editar_album_duplicado(self, album_id, titulo, anio, descripcion, medio):
        """
        Función duplicada para editar un álbum, con lógica idéntica a editar_album.
        """
        busqueda = (
            session.query(Album)
            .filter(Album.titulo == titulo, Album.id != album_id)
