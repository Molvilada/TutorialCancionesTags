...
class Ventana_Album(QWidget):
    ...
    def inicializar_ventana(self):
        ...
        self.boton_adicionar_cancion_existente = QPushButton(
            "Agregar canción existente"
        )
        self.boton_adicionar_cancion_existente.clicked.connect(
            self.mostrar_dialogo_agregar_cancion
        )
        ...
        for i in range(len(self.titulos_cancion)):
            ...
            self.caja_canciones.layout().addWidget(
                etiqueta, 0, i, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )
    ...
    def mostrar_album(self, album):
        ...
        self.lista_medios.setCurrentIndex(
            self.interfaz.dar_medios().index(album["medio"].name)
        )
    ...
    def mostrar_canciones(self, canciones):
        ...
        for cancion in canciones:
            ...
            texto_duracion = QLineEdit(
                "{}:{}".format(cancion["minutos"], cancion["segundos"])
            )
    ...
    def ver_cancion(self, cancion):
        ...
        self.interfaz.mostrar_ventana_cancion(
            id_album=self.album_actual["id"], id_cancion=cancion["id"]
        )
    ...
    def guardar_album(self):
        ...
        """
        album_modificado = {
            "titulo": self.texto_album.text(),
            "interpretes": self.texto_descripcion.text(),
            "medio": self.lista_medios.currentText(),
            "ano": self.texto_anio.text(),
            "descripcion": self.texto_descripcion.text(),
        }
        # Si hay campos vacios, se lanza un mensaje de error.
        if (
            album_modificado["titulo"] == ""
            or album_modificado["ano"] == ""
            or album_modificado["descripcion"] == ""
        ):
    ...
    def crear_cancion(self):
        ...
        self.hide()
        self.interfaz.mostrar_ventana_cancion(
            nueva=True, id_album=self.album_actual["id"]
        )
    ...
    def mostrar_dialogo_agregar_cancion(self):
        ...
        caja_botones.layout().addWidget(
            butAceptar, 0, 0, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
        )
        caja_botones.layout().addWidget(
            butCancelar, 0, 1, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
        )

        layout.addWidget(caja_botones, 3, 0, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter)

        butAceptar.clicked.connect(
            lambda: self.asociar_cancion_a_album(lista_canciones.currentData())
        )
